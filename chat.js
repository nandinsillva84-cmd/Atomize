...
(function () {
  ...
  if (!document.getElementById('chat-styles')) {
    ...
    style.textContent =
      ...
      '.chat-header-inline{display:flex;align-items:center;gap:10px;padding:10px 16px;background:#fff;border-radius:24px 24px 0 0;position:relative}' +
      '.chat-close-btn{background:none;border:none;font-size:18px;color:#888;cursor:pointer;padding:4px;margin-left:auto;}' +
  ...
  window.renderChatList = async function () {
    if (activeContactId) {
      ...
      // Não remove classe do cartão, pois ele não esconde mais
    ...
    for (var i = 0; i < chatContacts.length; i++) {
      (function (contact) {
        ...
        card.addEventListener('click', function () {
          ...
          // Insere o chat expandido logo após o cartão
  ...
  function createChatExpanded(contactId) {
    ...
    container.innerHTML =
      '<div class="chat-header-inline">' +  // Sem evento de clique que fecha
        ...
        '</div>' +
        // Seta de fechar (agora à esquerda do menu)
        '<button class="chat-close-btn" id="chatCloseBtn_' + contactId + '"><i class="fas fa-chevron-up"></i></button>' +
        // Menu três pontinhos (sem onclick inline)
        '<div style="position:relative;">' +
          '<i class="fas fa-ellipsis-v" id="chatMenuBtn_' + contactId + '" style="cursor:pointer;color:#888;font-size:18px;padding:4px;"></i>' +
          '<div class="chat-menu-dropdown" id="chatMenu_' + contactId + '">' +
            '<button id="changeBgBtn_' + contactId + '">🖼️ Mudar fundo</button>' +
          '</div>' +
        '</div>' +
      '</div>' +
    ...
    container.querySelector('#chatCloseBtn_' + contactId).addEventListener('click', function (e) {
      e.stopPropagation();
      closeChat(contactId);
    });

    // Evento do menu três pontinhos (com stopPropagation)
    var menuBtn = container.querySelector('#chatMenuBtn_' + contactId);
    menuBtn.addEventListener('click', function (e) {
      e.stopPropagation();
      window.toggleChatMenu(contactId);
    });

    // Impede que cliques dentro do menu dropdown fechem o chat
    var menuDropdown = container.querySelector('#chatMenu_' + contactId);
    menuDropdown.addEventListener('click', function (e) {
      e.stopPropagation();
    });

    // Botão "Mudar fundo"
    container.querySelector('#changeBgBtn_' + contactId).addEventListener('click', function (e) {
      e.stopPropagation();
      window.triggerChatBackground(contactId);
    });
  ...
  function closeChat(contactId) {
    ...
    setTimeout(function () {
      ...
      // Cartão permanece visível – não removemos classe alguma
      activeContactId = null;
      isAnimating = false;
    }, 500);
  }
  ...
  console.log('💬 Chat (textz) com cartão fixo, microfone, galeria e câmera carregado.');
})();