// ==================== app.js – YOU (Completo e Corrigido) ====================
(function () {
  ...
  window.confirmarSair = function () {
    var overlay = document.getElementById('confirmSairOverlay');
    if (overlay) overlay.style.display = 'flex';
  };
  window.fecharDialogoSair = function () {
    var overlay = document.getElementById('confirmSairOverlay');
    if (overlay) overlay.style.display = 'none';
  };
  window.executarSair = async function () {
    try {
      if (auth.currentUser && typeof setUserStatus === 'function') {
        await setUserStatus(auth.currentUser.uid, 'offline');
      }
      window.dispatchEvent(new CustomEvent('friendshipCleanup'));
      if (window.ATHOM_CACHE && window.ATHOM_CACHE.clearCurrentUser) {
        window.ATHOM_CACHE.clearCurrentUser();
      }
      localStorage.removeItem('you_tema');
      document.documentElement.style.setProperty('--bg-header-top', '#8b1031');
      document.documentElement.style.setProperty('--nav-bg', '#8b1031');
      await auth.signOut();
      window.fecharDialogoSair();
      window.showToast('Você saiu.');
    } catch (e) {
      window.showToast('Erro ao sair.');
    }
  };
  ...
  window.setActiveTab = function (tabName) {
    ...
    else if (tabName === 'textz' && typeof window.renderChatList === 'function') window.renderChatList();
  };
  ...
  auth.onAuthStateChanged(async function (user) {
    ...
    if (user) {
      ...
      // Painel Admin (apenas para o dono)
  ...
  console.log('📱 App YOU completo e corrigido.');
})();