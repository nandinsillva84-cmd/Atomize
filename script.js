...
async function setupExhibitionTab() {
    try {
        const vipPostsSnap = await db.collection('posts').where('media', '!=', null).orderBy('media').orderBy('createdAt','desc').limit(10).get();
        const vipPosts = [];
        for (const doc of vipPostsSnap.docs) {
            ...
            try {
                ...
                if (authorDoc.exists) {
                    ...
                    if (author.isVIP || author.isVerified) vipPosts.push({ id: doc.id, ...p });
                }
            } catch (e) {}
        }
        buildCarousel(vipPosts);
        loadOnlineFriends();
        const textPostsSnap = await db.collection('posts').orderBy('createdAt','desc').limit(20).get();
        const textPosts = [];
        textPostsSnap.forEach(doc => {
            const p = doc.data();
            if (!p.media || p.quote) textPosts.push({ id: doc.id, ...p });
        });
        renderExhibition(textPosts);
    } catch (e) {
        console.error('Erro na exposição:', e);
    }
}
