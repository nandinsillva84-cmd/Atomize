...
function switchTab(tab, el) {
    ...
    if (tab === 'feed') { currentFilter = 'status'; loadStatusFeed(); }
    else if (tab === 'exhibition') { currentFilter = 'exhibition'; setupExhibitionTab(); }
    else if (tab === 'chats') { loadChatList(); }
}
...
// ==================== ABA EXPOSIÇÃO ====================
async function setupExhibitionTab() {
    const vipPostsSnap = await db.collection('posts')
        .where('media', '!=', null)
        .orderBy('media')
    ...
    const vipPosts = [];
    for (const doc of vipPostsSnap.docs) {
        const p = doc.data();
        const authorDoc = await db.collection('users').doc(p.userId).get();
        if (authorDoc.exists) {
            const author = authorDoc.data();
            if (author.isVIP || author.isVerified) {
                vipPosts.push({ id: doc.id, ...p });
            }
        }
    }

    buildCarousel(vipPosts);
    loadOnlineFriends();

    const textPostsSnap = await db.collection('posts')
        .orderBy('createdAt', 'desc')
        .limit(20)
        .get();

    const textPosts = [];
    textPostsSnap.forEach(doc => {
        const p = doc.data();
        if (!p.media || p.quote) {
            textPosts.push({ id: doc.id, ...p });
        }
    });

    renderExhibition(textPosts);
}

function renderExhibition(posts) {
    const wrapper = document.getElementById('exhibitionPosts');
    wrapper.innerHTML = '';
    if (!posts.length) { wrapper.innerHTML = '<div style="color:#fff;text-align:center;margin-top:50px;">Nenhum post em exposição.</div>'; return; }
    posts.forEach(p => {
...
function buildCarousel(posts) {
    ...
    track.innerHTML = posts.map(p => `<div class="carousel-slide" onclick="showToast('Post de ${p.name}')"><img src="${p.avatar}" onerror="this.src='https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?ixlib=rb-4.0.3&auto=format&fit=crop&w=80&q=80'"><div class="carousel-text"><h4>${p.name}</h4><p>${p.quote?.substring(0,60)||''}</p></div></div>`).join('');
...
async function openFriendProfile(uid) {
    ...
    const profileHTML = `
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 8px;">
            ...
            ${medias.map(m => `
                <img src="${m.media}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px;">
            `).join('')}
        </div>
    `;
...
function openContactsModal() {
    ...
    Promise.all(friends.map(async uid => {
        ...
        contacts.forEach(c => { html += `<div class="contact-item" onclick="openFriendProfile('${c.uid}')"><img src="${c.avatar}" class="contact-avatar"><div><b>${c.name}</b></div></div>`; });
        db.collection('groups').where('members','array-contains',currentUid).get().then(gSnap => {
            if (!gSnap.empty) { html += '<h4>Grupos</h4>'; gSnap.forEach(doc => { const g = doc.data(); html += `<div class="contact-item" onclick="openGroupChat('${doc.id}','${g.name}')"><i class="fas fa-users"></i><b>${g.name}</b></div>`; }); }
            list.innerHTML = html;
        });
    });
}
async function openExploreModal() {
    ...
    list.innerHTML = users.map(u => `<div class="contact-item" onclick="openFriendProfile('${u.id}')"><img src="${u.avatar}" class="contact-avatar"><div><b>${u.name}</b></div></div>`).join('');
}
...
async function editUser(uid) {
    ...
    if (name !== null) {
        await db.collection('users').doc(uid).update({ name, email, password, avatar, status, quote });
        showToast('Usuário atualizado completamente.');
...
async function viewUserChats(uid) {
    ...
    chatsSnap.forEach(doc => {
        if (doc.id.includes(uid)) chatList += `Chat: ${doc.id.replace(/_/g, ' e ')}\n`;
    });
...
async function loadAdminPosts(area) {
    ...
    posts.forEach(doc => {
        const p = doc.data();
        area.innerHTML += `<div style="display:flex;justify-content:space-between;padding:10px;border-bottom:1px solid #ccc;"><span><b>${p.name}</b>: ${p.quote?.substring(0,40)}</span><button class="btn-small btn-danger" onclick="deletePost('${doc.id}')">Excluir</button></div>`;
    });
}
...
async function loadAdminChats(area) {
    ...
    chats.forEach(chat => {
        area.innerHTML += `<div style="display:flex;justify-content:space-between;padding:10px;border-bottom:1px solid #ccc;"><span>Chat: ${chat.id.replace(/_/g,' e ')}</span><button class="btn-small btn-danger" onclick="clearChatAdmin('${chat.id}')">Limpar</button></div>`;
    });
}
async function clearChatAdmin(chatId) {
    if (confirm('Limpar chat?')) {
        ...
        const batch = db.batch(); snap.forEach(doc => batch.delete(doc.ref));
        await batch.commit(); showToast('Chat limpo.'); loadAdminTabContent('chats');
    }
}
...
async function saveTurbinate() {
    const data = {
        avatar: document.getElementById('turbAvatar').value.trim(),
        hasCrown: document.getElementById('turbHasCrown').value === 'true',
        crownColor: document.getElementById('turbCrownColor').value,
        frameColor: document.getElementById('turbFrameColor').value,
        frameStyle: document.getElementById('turbFrameStyle').value,
        isVIP: document.getElementById('turbIsVIP').value === 'true'
    };
...
async function saveDevConfig() {
    const settings = {
        primaryColor: document.getElementById('devPrimaryColor').value,
        appBackground: document.getElementById('devAppBg').value.trim(),
        chatBackground: document.getElementById('devChatBg').value.trim(),
        tabOrder: document.getElementById('devTabOrder').value.trim(),
        hideNow: document.getElementById('devHideNow').checked,
        hideTextz: document.getElementById('devHideTextz').checked,
        hideExhibition: document.getElementById('devHideExhibition').checked,
        maxPostsPerDay: parseInt(document.getElementById('devMaxPosts').value)||10,
        vipCost: parseInt(document.getElementById('devVipCost').value)||200
    };
...
async function loadAdminFeedback(area) {
    ...
    if (snaps.empty) { area.innerHTML += '<p>Nenhum feedback enviado.</p>'; return; }
    snaps.forEach(doc => {
        const f = doc.data();
        const date = f.createdAt ? new Date(f.createdAt.toDate()).toLocaleString() : '';
        area.innerHTML += `<div style="padding:10px;border-bottom:1px solid #ccc;">
            <b>${f.userName} (${f.userEmail})</b><br>
            <span style="font-size:13px;color:#555;">${f.text}</span><br>
...
function initApp() {
    loadGlobalConfig().then(() => {
        ...
        else if (activeTab === 'exhibition') setupExhibitionTab();
